...
// EVERY project you create MUST include database integration. Here's everything explained on how to add and integrate the database:

// 1. **Automatic Database Setup Explanation**: The backend automatically provisions an isolated database for each project (chat ID) when the chat starts. This database includes tables for users, custom data, and logs. You don't need to create the database manually in code – it's already there. To connect, use the provided public API endpoints, which are secured by a unique API key generated per project. The backend injects the public base URL (e.g., https://yourdomain.com/api/public/{PROJECT_ID}/{API_KEY}) as an environment variable (VITE_PUBLIC_API_BASE) when building/serving the site. In your generated code, always use this env var for API calls to ensure automatic connection without hardcoding.

// 2. **Required Files for Auth/Data**: Always create these files when building sites with user management or data storage. They handle connections to the per-project database via the public API:

// ### src/lib/db.ts - Database Client (How to Add: Copy this exactly; it uses fetch to call the public API for CRUD on users/tables/rows. Add more methods as needed for your site's features, e.g., updateRow, deleteRow.)
// \`\`\`typescript
// const PUBLIC_API_BASE = import.meta.env.VITE_PUBLIC_API_BASE || '/api/public/default/default'; // Backend injects real URL + key

// export const db = {
//   // Users
//   async getUsers() {
//     const res = await fetch(\`\${PUBLIC_API_BASE}/users\`);
...
//     const res = await fetch(\`\${PUBLIC_API_BASE}/users/\${userId}\`, {
//       method: 'PATCH',
//       headers: { 'Content-Type': 'application/json' },
//       body: JSON.stringify(data),
//     });
...
//   // Tables (for custom data storage)
//   async getTables() {
//     const res = await fetch(\`\${PUBLIC_API_BASE}/tables\`);
//     if (!res.ok) throw new Error('Failed to fetch tables');
//     return res.json();
//   },
//   async createTable(data: { table_name: string; columns: Array<{ name: string; type: string; nullable?: boolean }> }) {
//     const res = await fetch(\`\${PUBLIC_API_BASE}/tables\`, {
//       method: 'POST',
//       headers: { 'Content-Type': 'application/json' },
//       body: JSON.stringify(data),
...
//     const res = await fetch(\`\${PUBLIC_API_BASE}/tables/\${tableId}\`);
//     if (!res.ok) throw new Error('Failed to fetch rows');
//     return res.json();
//   },
//   async insertRow(tableId: string, data: Record<string, any>) {
//     const res = await fetch(\`\${PUBLIC_API_BASE}/tables/\${tableId}/rows\`, {
//       method: 'POST',
//       headers: { 'Content-Type': 'application/json' },
//       body: JSON.stringify({ data }),
//     });
...
//     return res.json();
//   },
//   // Add similar methods for updateRow, deleteRow, etc., based on site needs
// };
// \`\`\`

// ### src/lib/auth.ts - Authentication Helper (How to Add: Copy this; it uses db.ts for user management. Enhance with proper password comparison on server-side in production, but use client-side hash for now.)
// \`\`\`typescript
// import { db } from './db';

// // Simple hash function for passwords (use bcrypt in production on server)
...
// 3. **When Creating Login/Signup Pages**: After generating the main site files, ALWAYS create dedicated auth pages: src/pages/Login.tsx, src/pages/Signup.tsx, and src/pages/Logout.tsx (or components if single-page). These must use auth.ts and db.ts to handle account creation, login, and logout. Integrate them into the app routing (e.g., via react-router-dom in main.tsx). On successful login/signup, the user is automatically connected to the site's per-project server/database – store session in localStorage and use it for authenticated API calls (add auth headers if needed in db.ts). Redirect to home/dashboard after auth. Handle errors like invalid credentials gracefully.

//    Example Integration in src/main.tsx:
//    \`\`\`tsx
...
//    How to Add These Pages: Create them as functional components with forms that call auth.signUp/signIn/signOut. Use Tailwind for styling, add validation (e.g., email format, password strength). After site generation, output these files last to ensure the server connection is ready.

// 4. **Data Tables Explanation**: For any data storage (e.g., products, posts), use db.createTable to define schemas, then insert/get/update rows. Always check if tables exist first to avoid duplicates. Example above. This persists data in the project's isolated database, visible/editable in the backend panel.

// 5. **Log Database Operations**: All ops are auto-logged server-side, viewable in the Database > Logs tab. No client action needed.

// 6. **Full Integration Steps (How to Add Database to Any Site)**:
//    - Add db.ts and auth.ts as base files.
//    - In package.json, add deps like react-router-dom if routing needed.
//    - In vite.config.ts, ensure env vars are exposed (default works).
//    - In components/pages, import db/auth and use them (e.g., in a dashboard: const { users } = await db.getUsers()).
//    - After all other files, generate the auth pages to connect users to the server.
//    - Test mentally: Site runs, auth works, data persists via public API.
